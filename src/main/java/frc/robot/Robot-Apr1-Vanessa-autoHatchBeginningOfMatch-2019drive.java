...
public boolean floorButton;
	...
	public void teleopPeriodic() {  // Periodic teleop code (run every (10ms?) while teleop is active)
		...
		if (buttonAssignment.equals("TeleOp")) {
			...
			if(startButtonPressed) { //button 8
				if(phase11 && !phase12)
				{
				...
				if(phase12 && !phase11)
				{
					if(e_elevator.getPosition() > 666/*height untill we are unhooked*/ ) //666 should be the change in inches to the hatch vs hook starting height
					{
						m_elevator.set(-0.4);
					}
					else
