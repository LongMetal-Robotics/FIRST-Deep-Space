...
/* Code after WNEU - Thursday, 28rd March, 2019. 6:27pm                 */
...
public boolean floorButton;
	...
	UsbCamera camera2 = CameraServer.getInstance().startAutomaticCapture();
			// edit the _LO to _HI to increase resolution.....or vice versa
	camera2.setResolution(IMG_WIDTH_HI, IMG_HEIGHT_HI);
			// trying to enforce FPS limit.. please use this method.
	camera2.setFPS(15);
			// we played through all of the pixel formats and "kYUYV" is the best at conserving bandwidth for our microsoft camera.. subject to change if required.
	...
	public void teleopPeriodic() {  // Periodic teleop code (run every (10ms?) while teleop is active)
		...
		if (armGamepad.getRawButton(8) == true) {startButtonPressed = true;} else {startButtonPressed = false;} //start button, so the auto code
		if (armGamepad.getRawButton(7) == true) {selectButtonPressed = true;} else {selectButtonPressed = false;}
		...
		if (buttonAssignment.equals("TeleOp")) {
			...
			boolean phase00 = false;
			boolean phase10 = true;
			boolean phase11 = true;
			boolean phase12= false;
			...
			if(phase00) { 
				...
				if(phase11 && !phase12)
				{
					if(e_elevator.getPosition() < 3/*height untill we are unhooked*/ )
					{
						m_elevator.set(0.4);
					}
					else
				...
				if(phase12 && !phase11)
				{
					if(e_elevator.getPosition() > -5 ) //-5 should be the change in inches to the hatch vs hook starting height
					{
						m_elevator.set(-0.4);
					}
					else
				...
				if(phase10)
				{
					...
					// 27 ft (hlaf of field) - 4ft(hab level 2 platform) - 7ft 4.25in (length of cargo) = back of level 1 hab to front face of cargo 15ft 7.75in
