...
boolean phase12;
	...
	public double limiter(double min, double max, double value)
	{
		double newvalue;
		if(value < min)
		{
	...
	public void teleopPeriodic() {  // Periodic teleop code (run every (10ms?) while teleop is active)
		...
		if (buttonAssignment.equals("TeleOp")) {
			...
			/*if (controlType.equals("Tank")) {
				...
				driveX = driveX + limiter(-0.04, 0.04, limiter(-0.7, 0.7, -driveStickLeft.getY()) - driveX);
				//driveZ = driveZ + limiter(-0.15, 0.15, driveStickRight.getZ() - driveZ);
