...
public class Robot extends TimedRobot {
	...
	*/
	...
	public void robotInit() {	// Run when robot is turned on. Initialize m.controllers, etc.
		...
		CANSparkMax elevator = new CANSparkMax(4, MotorType.kBrushless); //for elevator
	...
	public void teleopPeriodic() {	// Periodic teleop code (run every (10ms?) while teleop is active)
        ...
        */
		...
		if (armGamepad.getRawAxis(3) > 0 && armGamepad.getRawAxis(2) == 0) {
			tallDrive.set(armGamepad.getRawAxis(3));
		} else if (armGamepad.getRawAxis(2) > 0 && armGamepad.getRawAxis(3) == 0) {
			tallDrive.set(-armGamepad.getRawAxis(2));
		} else {
			tallDrive.set(0.0);
		}
		armGamepad.getRawAxis(2); //left trigger
		armGamepad.getRawAxis(3); //right trigger
