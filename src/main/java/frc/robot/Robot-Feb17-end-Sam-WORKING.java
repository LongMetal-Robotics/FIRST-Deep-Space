...
public class Robot extends TimedRobot {
	...
	*/
	...
	public void robotInit() {	// Run when robot is turned on. Initialize m.controllers, etc.
		...
		climbBack = new DoubleSolenoid(1, 0); //parameters are in and out channels
		climbFront = new DoubleSolenoid(3, 2);
		...
		matchPhase.addOption("Already Climbed (Safety)", "Disable");
	...
	public void teleopPeriodic() {	// Periodic teleop code (run every (10ms?) while teleop is active)
		...
		controlType=controls.getSelected();
		String phase = matchPhase.getSelected();
        ...
        if (armGamepad.getRawButton(6)== true) {R1Pressed = true;} //front piston up on elevator (L2)
       ...
       if(downArrowPressed)
       {
        ...
        if(buttonYPressed) //button for climb pistons down
        {
			...
			// timer.start();
  //put both climb soleniods extended
			// if(timer.get() > 5)
			// {
