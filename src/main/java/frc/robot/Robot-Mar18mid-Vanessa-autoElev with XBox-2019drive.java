...
public class Robot extends TimedRobot {
	...
	public void teleopPeriodic() {  // Periodic teleop code (run every (10ms?) while teleop is active)
		//button boi hours
        if (armGamepad.getPOV(0) == 180) {downArrowPressed = true; floorButton = true;} else {downArrowPressed = false;} //elevator down (down arrow)
		if (armGamepad.getPOV(0) == 0) {upArrowPressed = true; tallButton = true;} else {upArrowPressed = false;} //elevator up (up arrow)
		rightArrowPressed_old = rightArrowPressed;
		if (armGamepad.getPOV(0) == 90) {rightArrowPressed = true; midButton = true;} else {rightArrowPressed = false;} // elevator next target up
		leftArrowPressed_old = leftArrowPressed;
		if (armGamepad.getPOV(0) == 270) {leftArrowPressed = true; lowButton = true;} else {leftArrowPressed = false;} // elevator next target down
		...
		if (armGamepad.getRawButton(8) == true) {autoElev = true;} else {autoElev = false;}
...
*/
			...
			double RTValue = armGamepad.getRawAxis(3); //right trigger      drive forwards
			if (RTValue > 0 && LTValue == 0) {
